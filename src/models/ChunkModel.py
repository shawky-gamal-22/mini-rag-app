...
class ChunkModel(BaseDataModel):
    ...
    async def create_chunk(self, chunk: DataChunk) -> DataChunk:
        ...
        :param chunk: DataChunk object to be created.
        :return: Created DataChunk object.
        """
    ...
    async def get_chunk(self, chunk_id: str) -> DataChunk:
        """
        Get a data chunk by its ID.

        :param chunk_id: ID of the data chunk to retrieve.
        :return: DataChunk object if found, None otherwise.
        """
        async with self.db_client() as session:
            result = await session.execute(DataChunk).where(DataChunk.chunk_id == chunk_id)
